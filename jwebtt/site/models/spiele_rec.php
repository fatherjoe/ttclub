...
class WebttModelSpiele_rec extends JModelItem
{
        ...
        public function getXML()
        {
				...
				$this->WebttHelperSpiele_rec = new WebttHelperSpiele_rec;
				$this->WebttHelperHallen = new WebttHelperHallen;

				// Wenn Aktualisierungsintervall abgelaufen ist, dann aktualisieren
				if ($this->WebttHelper->update_test_verein('spiele_rec') === TRUE)
				{
						$update = $this->WebttHelperSpiele_rec->update();
				}
        ...
        public function getXMLPopups()
        {
				...
				foreach ($xml->ZEILE as $zeile)
				{
						if (trim($zeile->ERGEBNIS_PFAD))
						{
								...
								{
//										$this->WebttHelperSpielplaene->update_popup($meeting,$zeile->ERGEBNIS_PFAD,$zeile->STAFFEL);
								}
				...
				{
						if ($idsclicktt && array_search($result->idclicktt, $idsclicktt))
						{ 
		...
		public function getHtmlPopupFiles()
		{
				...
				foreach($xmlPopups as $meeting => $xml)
				{
						...
						$html .= '<link rel="stylesheet" href="' . JURI::base() . 'media/com_webtt/css/spielpl_verein.css" type="text/css" />';
