...
class WebttHelperPokaluebersicht extends WebttHelper
{
		...
		public function update()
		{
				...
				if ($page)
				{
						...
						foreach ($rows as $tr)
						{
								...
								if ($tds->length == 5 && stristr($tds->item(1)->nodeValue, "Pokal"))
								{
						...
						$query
							...
							->where(array( 'typ=' . $db->quote('pokaluebersicht')));
						...
						if (isset($db->loadObject()->datum))
						{
								...
								$conditions = array(
									$db->quoteName('typ') . ' = ' . $db->quote('pokaluebersicht')
								);
		...
		public function getTeamsPokalStaffeln()
		{
                ...
                $query
                    ->select('*')
                    ->from('#__webtt_teams')
                    ->where($db->quoteName('typ') . ' = ' . $db->quote('pokal'));
