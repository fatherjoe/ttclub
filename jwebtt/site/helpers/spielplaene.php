...
class WebttHelperSpielplaene extends WebttHelper
{
		...
		public function update()
		{
				...
				$post = 	"searchTimeRange=1" . 
							"&searchType=1" . 
							"&searchTimeRangeFrom=01.08." . $saison_hin . 
							"&searchTimeRangeTo=31.05." . $saison_rueck . 
							"&club=" . $verein_nr . 
		...
		public function update_popup($meeting,$path)
		{
				...
				if ($meetingpage)
				{
						...
						{ 
								...
								// Spieldaten aus dem Spielbericht lesen: Staffel und Teams
								$h1 = $dom->getElementsByTagName('h1');
								foreach ($h1->item(0)->childNodes as $h1sub)
								{
										$h1List[] =  $h1sub->textContent;
								}
								$staffel = WebttHelper::trimClickttField($h1List[2]);
								$temp = explode(",", $h1List[4]);

								$teams = explode("-", $temp[0]);
								$heim = trim($teams[0]);
								$gast = trim($teams[1]);
				...
				else
				{
						...
						$fields = array(
							...
							$db->quoteName('typ') . ' = ' . $db->quote('meeting'),
							$db->quoteName('idclicktt') . ' = ' . $db->quote($meeting),
							$db->quoteName('name') . ' = ' . $db->quote($heim . " - " . $gast),
