...
class LeagueTable extends Table
{
    /**
     * Constructor.
     *
    ...
    {
        parent::__construct('#__ttclub_leagues', 'id', $db);
    }
    ...
    /**
     * Override delete to prevent deletion when teams reference this league.
     *
    ...
    public function delete($pk = null): bool
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('league_id') . ' = ' . (int) $pk);

        $db->setQuery($query);
        $teamCount = (int) $db->loadResult();

        if ($teamCount > 0) {
            $this->setError('Cannot delete this league because it still has associated teams.');
            return false;
        }
